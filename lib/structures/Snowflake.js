...
class SnowflakeUtil {
   ...
   */
	static deconstruct(snowflake) {
		const BINARY = this.idToBinary(snowflake).toString(2).padStart(64, '0');
		const res = {
   ...
   * @private
	...
	static idToBinary(num) {
		let bin = '';
		let high = Number(num.slice(0, -10)) || 0;
		let low = Number(num.slice(-10));
		while (low > 0 || high > 0) {
			bin = String(low & 1) + bin; // eslint-disable-line no-bitwise
