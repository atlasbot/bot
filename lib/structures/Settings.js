 ...
 * @returns {Promise} the MongoDB update promise
 */
 ...
 */
	...
	findWarning(query) {
		return (new this.Atlas.structs.Fuzzy(this.settings.plugins.moderation.infractions, {
			keys: ['reason'],
		})).search(query);
	}
 ...
 */
	command(label) {
		...
		const opts = this.settings.cmd_opts.find(o => o.label === label);
 ...
 */
	async getTicketCat() {
		if (this.module('tickets').disabled) {
 ...
 */
	getTickets(user) {
		return this.module('tickets').tickets
